...
# Append transgene FASTAs (TRANS_FA is built by cr-mkref create)
: "${TRANS_FA:?TRANS_FA is required (path to concatenated transgene FASTA)}"
echo "appending transgene FASTA from $TRANS_FA"
...
TRANS_GTF="$(dirname "$YAML_PATH")/trans.gtf"
